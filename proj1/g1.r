...
eigen_values <- eigen(cov(dat))$values
dat_var_per <- eigen_values / sum(eigen_values) * 100
dat_var_per

eigen_values_std <- eigen(cor(dat))$values
dat_vat_per_std <- eigen_values_std / sum(eigen_values_std) * 100
dat_vat_per_std

library(psy)
dat_cc
third_component <- eigen_vectors[, 3]
scree.plot(dat_stand,title="Stdardized",type="R")
scree.plot(third_component, title= "Classic", type="R")
